...
class Dier(db.Model):
    ...
    achternaam = db.Column(db.String(45), nullable=True)
    ...
    opnamen_id = db.Column(db.Integer, db.ForeignKey('opnamen.id'), nullable=False)
    gebied = db.relationship('Gebied', backref='author', lazy=True)
    opnamen = db.relationship('Opnamen', backref='author2', lazy=True)
...
class Gebied(db.Model):
    ...
    def __repr__(self):
        return f"Gebied('{self.naam}', '{self.soort}', '{self.grootte}')"
...
class Opnamen(db.Model):
    ...
    def __repr__(self):
        return f"Opnamen('{self.naam}', '{self.kwaliteit}', '{self.duur}', '{self.current_time}')"
